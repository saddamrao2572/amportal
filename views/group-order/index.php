...
if (\Yii::$app->user->can('app_groups_view', ['route' => true])) {
    $actionColumnTemplates[] = '{view}';
}

if (\Yii::$app->user->can('app_groups_update', ['route' => true])) {
    $actionColumnTemplates[] = '{update}';
}

if (\Yii::$app->user->can('app_groups_delete', ['route' => true])) {
...
?>
<div class="giiant-crud groups-index">
		...
		<div class="col-12 col-lg-12">
			<div class="mb-2">
				...
				<div class="float-md-right text-zero">
				   <a href="<?=url::to(['groups/create'])?>" class="btn btn-primary default btn-lg mb-1" style="font-size: 12px"> <i class="simple-icon-plus"></i> Add New groups</a>
                ...
                <div class="card-body">
                    <div class="dashboard-quick-post">
						...
						'urlCreator' => function ($action, $model, $key, $index) {
							if ($action === 'edit') {
								$url = Url::to(['/groups/update','id'=>$model->id]); // your own url generation logic
								return $url;
							}
							if ($action === 'delete') {
								$url = Url::to(['/groups/delete','id'=>$model->id]); // your own url generation logic
								return $url;
							}
							
							if ($action === 'view') {
								$url = Url::to(['/groups/view/','id'=>$model->id]); // your own url generation logic
								return $url;
							}
